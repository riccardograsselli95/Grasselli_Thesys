...
def subsample_image(dir1, dir2, image_path, save_path, percent, new_name, visualize):
    ...
    space_x = int(crow / numb_of_spoke)   
    space_y = int(ccol / numb_of_spoke)
    points_x = np.arange(0, rows, space_x, dtype=int)
    points_y = np.arange(0, cols, space_y, dtype=int)
    if numb_of_spoke != 8:
        points_x[-1] = 368
        points_y[-1] = 368
    ...
    for j in points_x:
        ...
        if j == 0:
            for i in range(j,j+width_1):
                mask = draw_line(mask, i,points_y[0],int(points_xflip[0]-i), points_yflip[0], inplace=False)
        elif j == (rows-width_2):
    ...
    for j in points_y:
        ...
        if j == 0:
            for i in range(j,j+width_1):
                #print('POINTS1: ', i,' - ', points_x[0],' - ', int(points_xflip[0]), ' - ', points_yflip[0]-i)
                mask = draw_line(mask, points_x[0],i,points_xflip[0], int(points_yflip[0]-i), inplace=False)
        elif j == (cols-width_2):
            for i in range(j-width_2,j):
                #print('POINTS1: ', i,' - ', points_x[0],' - ', int(points_xflip[0]), ' - ', points_yflip[0]-i)
                mask = draw_line(mask, points_x[0],i,points_xflip[0], int(points_yflip[0]-i), inplace=False)
